...
class ProcessCSVCCG(processcsvs.ProcessCSV):
    def __init__(self, filename, dialect, row, headers, delimiter="|",response=None):
        super(ProcessCSVCCG, self).__init__(filename, dialect,response=response)
...
class CSVCCGValidated(pivalidatecsv.CSVPIValidated):
    def __init__(self,filename, dialect, row, headers, delimiter="|",response=None):
        super(CSVCCGValidated, self).__init__(filename, dialect,delimiter, headers,response=response)
    ...
    def write_row_data(self, coldata, val):
        try:
            ...
            self.gc_field = self.load_validated_field(self.validated_field.get("Guarantor_Classification"))
            self.gtt_field = self.load_validated_field(self.validated_field.get("Guarantee_Type"))
            self.gt_field = self.load_validated_field(self.validated_field.get("Guarantor_Type"))
            #self.bs_field = self.load_validated_field(self.validated_field.get("borrower_stake"))
            #print "REFERENCE ", self.bar_field
            ...
            for data in coldata:
                self.writer.writerow([
                                    ...
                                    self.value_in(smart_str(unicode(data.Guarantor_Classification)),self.gc_field),
                                    self.value_in(smart_str(unicode(data.Guarantee_Type)), self.gtt_field),
                                    self.value_in(smart_str(unicode(data.Guarantor_Type)),self.gt_field),
