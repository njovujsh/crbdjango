...
class ENF045(enf001.ENF001):
    def __init__(self, mobject, field, priority, action, value=5):
        super(ENF045, self).__init__(mobject, field, priority, action)
        self.value = value 
        
        self.risk = riskclassification.RiskClassification()
        
    def validate_field(self, field):
        try:
            self.fields = self.risk.search_value(field)
            if(self.fields):
                return True
            else:
                return False 
