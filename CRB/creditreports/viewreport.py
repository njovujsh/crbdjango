...
def load_modules(dataset):
    if(dataset):
        return loadmodels.load_model_module(dataset)
    else:
        return False 
        
#creport
class ViewValidationReport(View):
    @method_decorator(login_required)
    def get(self, request, record_slug):
        try:
            self.rendered_context = self.process_context(request, record_slug)
    ...
    def process_context(self, request, record_slug):
        try:
            #print "MODEL ", load_modules(record_slug)
            self.template = self.load_rules_template("viewvalidationreports.html")
            if(self.template):
                try:
