...
class SheetReaderThread(threading.Thread):
    """
    Threads responsible for reading sheets
    Threads will be executed based on number of
    sheets.
    """
    def __init__(self, xl_object, sheetname, name):
        super(SheetReaderThread, self).__init__(name=name)
    ...
    def run(self):
        ...
        try:
            ...
            #print "My mame is ", self.getName()
            #print "Attempting to processing", self.names.ncols  
            rows =  self.names.nrows 
            #for rows in range(len(self.names.nrows)):
            for r in range(rows):
                print self.xlobject.get_row_values(0, r)
